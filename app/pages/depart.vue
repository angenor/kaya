 ...
 * c'est **la langue** : sans elle, l'écran affichait « sejours.depart.chargement_impossible » — un message d'échec
 * technique, qui envoie chercher un problème de réseau qui n'existe pas.
 */
const peutOuvrir = computed(() => peutOuvrirEcran('/depart', permissions.value))
...
onMounted(async () => {
  ...
  if (!peutOuvrir.value) {
    erreur.value = t('sejours.depart.acces_refuse')
    return
  }
