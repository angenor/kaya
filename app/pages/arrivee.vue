 ...
 * c'est **la langue** : sans elle, l'écran affichait « sejours.arrivee.chargement_impossible » — un message d'échec
 * technique, qui envoie chercher un problème de réseau qui n'existe pas.
 */
const peutOuvrir = computed(() => peutOuvrirEcran('/arrivee', permissions.value))
...
onMounted(async () => {
  ...
  if (!peutOuvrir.value) {
    erreur.value = t('sejours.arrivee.acces_refuse')
    return
  }
